...
| LOC            | 420 (+ 2,965 bytes / 73 lines JS, 2,685 bytes / 81 lines PHP) |
...
- **Severity:** low
- **Category:** output-style
- **Location:** Install-SouliTEK.ps1 — line 271 (`"Step 1: Preparing..."`), 280 (`"Step 2: Downloading..."`), 310 (`"Step 3: Extracting..."`), 332 (`"Step 4: Checking for existing installation"`), 348 (`"Step 5: Installing essential files..."`), 371 (`"Step 6: Creating desktop shortcut"`), 392 (`"Step 7: Cleaning up temporary files"`). The source comments alongside disagree: lines 270, 279, 309, 331, 347, 370, 391 are commented `Step 1` / `Step 2` / `Step 3` / `Step 5` / `Step 6` / `Step 7` / `Step 8` — so the comment block jumps from `Step 3` (line 309) to `Step 5` (line 331) at the "Find the extracted folder" sub-step, and the user-facing string says `Step 4` while the source comment says `Step 5`.
- **Local notes:** Pure cosmetic — comments and user-visible step labels drifted apart over time. Either renumber the comments to match the user output (`Step 1..7`), or renumber the user output to include the "find extracted folder" sub-step. Recommend the former (less churn, doesn't change the user-facing UX). 30-minute fix.
- **Target phase:** P0
