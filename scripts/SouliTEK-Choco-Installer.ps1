<#
.SYNOPSIS
    SouliTEK WinGet Package Installer - Ninite-like UX for WinGet
.DESCRIPTION
...
.EXAMPLE
    .\SouliTEK-Choco-Installer.ps1 -Preset .\my-preset.json
...
function Write-Banner {
    ...
    Write-Host "  =========================================================" -ForegroundColor Cyan
    Write-Host ""
    Write-Host "  WinGet Package Installer - Ninite-like UX" -ForegroundColor White
...
function Initialize-Logging {
    try {
        ...
        $Script:TranscriptPath = Join-Path $Script:LogFolder "WinGetInstaller_$timestamp.log"
...
function Install-Office2024 {
    Write-Host "             [*] Downloading Office 2024 installer..." -ForegroundColor Cyan
    ...
    try {
        ...
        if (Test-Path $installerPath) {
            Write-Host "             [*] Installing Office 2024 (this may take a while)..." -ForegroundColor Cyan
        ...
        try {
            ...
            if ($isInstalled) {
                ...
                if ($pkgId -eq "OFFICE2024") {
                    ...
                    $wingetArgs = @("install", "-e", "--id", $pkgId, "--silent", "--accept-package-agreements", "--accept-source-agreements")
